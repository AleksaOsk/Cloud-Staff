...
class ErrorHandler {
    ...
    public ErrorResponse handleInvalidPhoneNumberException(final InvalidPhoneNumberException e) {
        log.error("InvalidPhoneNumberException: Пользователь указал некорректные данные. {}", e.getMessage());
    ...
    public ErrorResponse handlePhoneNumberAlreadyInUseException(final PhoneNumberAlreadyInUseException e) {
        log.error("PhoneNumberAlreadyInUseException: Пользователь указал некорректные данные. {}", e.getMessage());
        LocalDateTime now = LocalDateTime.now();
    ...
    //исключение при срабатывании аннотации на отдельных полях контроллера (переменные пути)
    ...
    @ExceptionHandler
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    //исключение при срабатывании аннотации на объектах(в dto)
